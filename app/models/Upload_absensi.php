...
class Upload_absensi extends CI_Model
{
	...
	public function up_absensi($filename,$idPer){		
		...
		$periode = $this->my_lib->get_data_row('master_periode',array('id_periode'=>$idPer));
		$worksheet = $objPHPExcel->getActiveSheet()->toArray(null,true,true,true);
		$numRows = count($worksheet);
		for ($i=1; $i < ($numRows+1) ; $i++) {
			$nip = $worksheet[$i]["D"];
			$kd_jamker = $this->my_lib->get_row('data_pegawai',array('nip'=>$nip),'kode_jam_kerja');
			$jamkerja = $this->my_lib->get_data_row('master_jam_kerja',array('kode_jam_kerja'=>$kd_jamker));
			$date = strtotime($worksheet[$i]["A"]); $new_date = date('Y-m-d',$date);
			$get_day = date("l", $date); $day_name = strtolower($get_day);
			...
			if ($day_name == "sunday") { //JIKA HARI MINGGU TIDAK DILAKUKAN PEMROSESAN				
				...
				if ($worksheet[$i]["B"] == "Tidak Hadir") { //JIKA TIDAK HADIR
					$in = '00:00:00'; $out = '00:00:00'; $lama_kerja = '00:00:00';
					$telat = 2;
		...
		foreach ($peg as $p) {
			$tot =0;
			$tepat_waktu = $this->my_lib->row_count('absensi_data',array('id_periode'=>$idPer,'nip'=>$p->nip, 'telat'=>0));
			$data_absensi = $this->my_lib->get_data('absensi_data',array('id_periode'=>$idPer,'nip'=>$p->nip));
			if (!empty($data_absensi)) {
				foreach ($data_absensi as $absensi) { //LOOPING DATA ABSENSI
					$tot += explode_time($absensi->lama_kerja);
				}
				$total_lama = convert_second($tot);
				$rata2 = $tot / $periode->row('pembagi');
				$new_rata2 = convert_second($rata2);
				if ($this->my_lib->cek('absensi_rekap',array('id_periode'=>$idPer,'nip'=>$p->nip)) == TRUE) {
					$rekap_update = array(
						"total_jam" => $total_lama,
						"rerata" => $new_rata2,
						"tepat_waktu" =>$tepat_waktu
					);
					$this->my_lib->edit_row('absensi_rekap',$rekap_update,array('id_periode'=>$idPer, 'nip'=>$p->nip));
				}
				else{
					$rekap_insert = array(
						"id_periode" => $idPer,
