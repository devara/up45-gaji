...
class Upload_absensi_backup extends CI_Model
{
    ...
    for ($i=1; $i < ($numRows+1) ; $i++) { #START LOOPING
    	$peg_nip = $worksheet[$i]["D"];
    	$cek_data = $this->my_lib->cek('absensi_rekap',array('id_periode'=>$idPer,'nip'=>$peg_nip));
    	if ($cek_data == FALSE) {
